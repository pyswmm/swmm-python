...
def test_object_find(handle):
    rg_index = solver.project_get_index(toolkit_enum.ObjectType.GAGE, 'RG1')
    sub_index_0 = solver.project_get_index(toolkit_enum.ObjectType.SUBCATCH, '2')
    sub_index_1 = solver.project_get_index(toolkit_enum.ObjectType.SUBCATCH, '3')
    node_index = solver.project_get_index(toolkit_enum.ObjectType.NODE, '24')
    link_index = solver.project_get_index(toolkit_enum.ObjectType.LINK, '16')
    assert rg_index == 0
...
def test_error_message_handling(handle):
    with pytest.raises(Exception) as exc_info:
        solver.project_get_index(toolkit_enum.ObjectType.SUBCATCH, 'sloth')

    assert 'API Key Error: Object index out of Bounds' in str(exc_info.value)


def test_object_count(handle):
    rg_count = solver.project_get_count(toolkit_enum.ObjectType.GAGE)
    sub_count = solver.project_get_count(toolkit_enum.ObjectType.SUBCATCH)
    assert rg_count == 1
    assert sub_count == 8
...
def test_simulation_datetime(handle):
    ...
    year, month, day, hour, minute, second = solver.simulation_get_datetime(toolkit_enum.TimeProperty.START_DATE)
    assert datetime(1998, 1, 1) == datetime(year, month, day, hour, minute, second)
    year, month, day, hour, minute, second = solver.simulation_get_datetime(toolkit_enum.TimeProperty.END_DATE)
    assert datetime(1998, 1, 2, 12) == datetime(year, month, day, hour, minute, second)
    year, month, day, hour, minute, second = solver.simulation_get_datetime(toolkit_enum.TimeProperty.REPORT_DATE)
    assert datetime(1998, 1, 1) == datetime(year, month, day, hour, minute, second)

    solver.simulation_set_datetime(toolkit_enum.TimeProperty.START_DATE, 2020, 1, 1, 12, 59, 59)
    year, month, day, hour, minute, second = solver.simulation_get_datetime(toolkit_enum.TimeProperty.START_DATE)
...
def test_object_get_index(handle):
    with pytest.raises(Exception) as exc_info:
        node_index = solver.project_get_index(toolkit_enum.ObjectType.NODE, '6')
        sub_index = solver.project_get_index(toolkit_enum.ObjectType.SUBCATCH, '20')

#    assert node_index == -1
#    assert sub_index == -1

    node_index = solver.project_get_index(toolkit_enum.ObjectType.NODE, '20')
    sub_index = solver.project_get_index(toolkit_enum.ObjectType.SUBCATCH, '6')
...
def test_object_get_id(handle):
    node_name = solver.project_get_id(toolkit_enum.ObjectType.NODE, 8)
    sub_name = solver.project_get_id(toolkit_enum.ObjectType.SUBCATCH, 5)
    assert node_name == '20'
    assert sub_name == '6'
...
def test_link_param(handle):
    offset_1 = solver.link_get_parameter(0, toolkit_enum.LinkProperty.OFFSET_1)
    offset_2 = solver.link_get_parameter(0, toolkit_enum.LinkProperty.OFFSET_2)
    initial_flow = solver.link_get_parameter(0, toolkit_enum.LinkProperty.INITIAL_FLOW)
    flow_limit = solver.link_get_parameter(0, toolkit_enum.LinkProperty.FLOW_LIMIT)
    ...
    solver.link_set_parameter(0, toolkit_enum.LinkProperty.FLOW_LIMIT, 1)
    solver.link_set_parameter(0, toolkit_enum.LinkProperty.INLET_LOSS, 0.5)
    solver.link_set_parameter(0, toolkit_enum.LinkProperty.OUTLET_LOSS, 1.0)
    solver.link_set_parameter(0, toolkit_enum.LinkProperty.AVERAGE_LOSS, 0.2)
...
def test_link_get_result(run_sim):
    flow = solver.link_get_result(0, toolkit_enum.LinkResult.FLOW)
    depth = solver.link_get_result(0, toolkit_enum.LinkResult.DEPTH)
    assert flow == 0.0
    assert depth == 0.0
...
def test_link_get_pollutant(run_sim):
    pollut = solver.link_get_pollutant(0, toolkit_enum.NodePollutant.QUALITY)
...
def test_pump_get_stats(run_sim):
    index = solver.project_get_index(toolkit_enum.ObjectType.LINK, 'P1')
    while True:
        time = solver.swmm_step()
        if time == 0:
            break
...
def test_node_get_type(handle):
    ...
    node_index = solver.project_get_index(toolkit_enum.ObjectType.NODE, '18')
...
def test_node_param(handle):
    ...
    solver.node_set_parameter(0, toolkit_enum.NodeProperty.INVERT_ELEVATION, 90)
    solver.node_set_parameter(0, toolkit_enum.NodeProperty.FULL_DEPTH, 10)
    solver.node_set_parameter(0, toolkit_enum.NodeProperty.SURCHARGE_DEPTH, 100)
    solver.node_set_parameter(0, toolkit_enum.NodeProperty.POND_AREA, 100)
    solver.node_set_parameter(0, toolkit_enum.NodeProperty.INITIAL_DEPTH, 1)
...
def test_node_get_result(run_sim):
    ...
    total_outflow = solver.node_get_result(0, toolkit_enum.NodeResult.TOTAL_OUTFLOW)
...
def test_node_get_pollutant(run_sim):
    pollut = solver.node_get_pollutant(0, toolkit_enum.NodePollutant.QUALITY)
    tss, lead = tuple(pollut)
    assert tss == 0.0
    assert lead == 0.0
...
def test_subcatch_param(handle):
    ...
    solver.subcatch_set_parameter(0, toolkit_enum.SubcatchProperty.WIDTH, 100)
    solver.subcatch_set_parameter(0, toolkit_enum.SubcatchProperty.AREA, 50)
    #solver.subcatch_set_parameter(0, toolkit_enum.SubcatchProperty.IMPERVIOUS_FRACTION, 100)
    solver.subcatch_set_parameter(0, toolkit_enum.SubcatchProperty.SLOPE, 0.1)
    solver.subcatch_set_parameter(0, toolkit_enum.SubcatchProperty.CURB_LENGTH, 10)
...
def test_subcatch_get_result(run_sim):
    rain = solver.subcatch_get_result(0, toolkit_enum.SubcatchResult.RAIN)
    evaporation = solver.subcatch_get_result(0, toolkit_enum.SubcatchResult.EVAPORATION)
    assert rain == 0.0
...
def test_subcatch_get_pollutant(run_sim):
    pollut = solver.subcatch_get_pollutant(0, toolkit_enum.SubcatchPollutant.BUILD_UP)
    tss, lead = tuple(pollut)
    assert tss == 0.0
    assert lead == 0.0
...
def test_lid_usage_parameter(lid_handle):
    ...
    initial_saturation = solver.lid_usage_get_parameter(0, 0, toolkit_enum.LidUsageProperty.INITIAL_SATURATION)
    from_impervious = solver.lid_usage_get_parameter(0, 0, toolkit_enum.LidUsageProperty.FROM_IMPERVIOUS)
    from_pervious = solver.lid_usage_get_parameter(0, 0, toolkit_enum.LidUsageProperty.FROM_PERVIOUS)
    ...
    solver.lid_usage_set_parameter(0, 0, toolkit_enum.LidUsageProperty.UNIT_AREA, 100)
    solver.lid_usage_set_parameter(0, 0, toolkit_enum.LidUsageProperty.TOP_WIDTH, 20)
    #solver.lid_usage_set_parameter(0, 0, toolkit_enum.LidUsageProperty.BOTTOM_WIDTH, 20)
...
def test_lid_group_get_result(run_lid_sim):
    index = solver.project_get_index(toolkit_enum.ObjectType.SUBCATCH, 'wBC')
    while True:
        time = solver.swmm_step()
        if time == 0:
            break
...
def test_lid_control_parameter(lid_handle):
    ...
    solver.lid_control_set_parameter(0, toolkit_enum.LidLayer.SURFACE, toolkit_enum.LidLayerProperty.THICKNESS, 12)
    surface_thickness = solver.lid_control_get_parameter(0, toolkit_enum.LidLayer.SURFACE, toolkit_enum.LidLayerProperty.THICKNESS)
    assert surface_thickness == 12


def test_gage_precipitation(run_sim):
    total = solver.raingage_get_precipitation(0, toolkit_enum.RainResult.TOTAL)
    snowfall = solver.raingage_get_precipitation(0, toolkit_enum.RainResult.SNOWFALL)
    rainfall = solver.raingage_get_precipitation(0, toolkit_enum.RainResult.RAINFALL)
    assert total == 0.0
    assert snowfall == 0.0
    assert rainfall == 0.0
