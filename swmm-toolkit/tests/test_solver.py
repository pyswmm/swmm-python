...
def test_get_object_id(handle):
    node_name = solver.get_object_id(toolkit_enum.ObjectProperty.NODE, 8)
    sub_name = solver.get_object_id(toolkit_enum.ObjectProperty.SUBCATCH, 5)
    assert node_name == '20'
    assert sub_name == '6'
...
def test_link_param(handle):
    offset_1 = solver.get_link_parameter(0, toolkit_enum.LinkProperty.OFFSET_1)
    offset_2 = solver.get_link_parameter(0, toolkit_enum.LinkProperty.OFFSET_2)
    initial_flow = solver.get_link_parameter(0, toolkit_enum.LinkProperty.INITIAL_FLOW)
    flow_limit = solver.get_link_parameter(0, toolkit_enum.LinkProperty.FLOW_LIMIT)
    inlet_loss = solver.get_link_parameter(0, toolkit_enum.LinkProperty.INLET_LOSS)
    outlet_loss = solver.get_link_parameter(0, toolkit_enum.LinkProperty.OUTLET_LOSS)
    average_loss = solver.get_link_parameter(0, toolkit_enum.LinkProperty.AVERAGE_LOSS)
    ...
    solver.set_link_parameter(0, toolkit_enum.LinkProperty.OFFSET_1, 0.2)
    solver.set_link_parameter(0, toolkit_enum.LinkProperty.OFFSET_2, 0.1)
    solver.set_link_parameter(0, toolkit_enum.LinkProperty.INITIAL_FLOW, 0.1)
    solver.set_link_parameter(0, toolkit_enum.LinkProperty.FLOW_LIMIT, 1)
    solver.set_link_parameter(0, toolkit_enum.LinkProperty.INLET_LOSS, 0.5)
    solver.set_link_parameter(0, toolkit_enum.LinkProperty.OUTLET_LOSS, 1.0)
    solver.set_link_parameter(0, toolkit_enum.LinkProperty.AVERAGE_LOSS, 0.2)
...
def test_get_link_result(run_sim):
    flow = solver.get_link_result(0, toolkit_enum.LinkResult.FLOW)
    depth = solver.get_link_result(0, toolkit_enum.LinkResult.DEPTH)
    assert flow == 0.0
    assert depth == 0.0
...
def test_get_link_pollutant(run_sim):
    pollut = solver.get_link_pollutant(0, toolkit_enum.NodePollutant.QUALITY)
...
def test_set_link_target_setting(run_sim):
    target_setting = solver.get_link_result(0, toolkit_enum.LinkResult.TARGET_SETTING)
...
def test_get_pump_stats(run_sim):
    index = solver.get_object_index(toolkit_enum.ObjectProperty.LINK, 'P1')
    while True:
        time = solver.step()
        if time == 0:
            break
...
def test_get_node_type(handle):
    ...
    node_index = solver.get_object_index(toolkit_enum.ObjectProperty.NODE, '18')
    node_type = solver.get_node_type(node_index)
    assert node_type == toolkit_enum.NodeType.OUTFALL.value

    
def test_node_param(handle):
    invert_elevation = solver.get_node_parameter(0, toolkit_enum.NodeProperty.INVERT_ELEVATION)
    full_depth = solver.get_node_parameter(0, toolkit_enum.NodeProperty.FULL_DEPTH)
    surcharge_depth = solver.get_node_parameter(0, toolkit_enum.NodeProperty.SURCHARGE_DEPTH)
    pond_area = solver.get_node_parameter(0, toolkit_enum.NodeProperty.POND_AREA)
    initial_depth = solver.get_node_parameter(0, toolkit_enum.NodeProperty.INITIAL_DEPTH)
    ...
    solver.set_node_parameter(0, toolkit_enum.NodeProperty.INVERT_ELEVATION, 90)
    solver.set_node_parameter(0, toolkit_enum.NodeProperty.FULL_DEPTH, 10)
    solver.set_node_parameter(0, toolkit_enum.NodeProperty.SURCHARGE_DEPTH, 100)
    solver.set_node_parameter(0, toolkit_enum.NodeProperty.POND_AREA, 100)
    solver.set_node_parameter(0, toolkit_enum.NodeProperty.INITIAL_DEPTH, 1)
...
def test_get_node_result(run_sim):
    total_inflow = solver.get_node_result(0, toolkit_enum.NodeResult.TOTAL_INFLOW)
    total_outflow = solver.get_node_result(0, toolkit_enum.NodeResult.TOTAL_OUTFLOW)
...
def test_get_node_pollutant(run_sim):
    pollut = solver.get_node_pollutant(0, toolkit_enum.NodePollutant.QUALITY)
    tss, lead = tuple(pollut)
    assert tss == 0.0
    assert lead == 0.0
...
def test_set_outfall_stage(run_sim):
    head = solver.get_node_result(13, toolkit_enum.NodeResult.HEAD)
    head == 975
...
def test_get_storage_stats(run_sim):
    node_index = solver.get_object_index(toolkit_enum.ObjectProperty.NODE, 'SU1')
    while True:
        time = solver.step()
        if time == 0:
            break
...
def test_subcatch_param(handle):
    width = solver.get_subcatch_parameter(0, toolkit_enum.SubcatchProperty.WIDTH)
    area = solver.get_subcatch_parameter(0, toolkit_enum.SubcatchProperty.AREA)
    impervious_fraction = solver.get_subcatch_parameter(0, toolkit_enum.SubcatchProperty.IMPERVIOUS_FRACTION)
    slope = solver.get_subcatch_parameter(0, toolkit_enum.SubcatchProperty.SLOPE)
    curb_length = solver.get_subcatch_parameter(0, toolkit_enum.SubcatchProperty.CURB_LENGTH)
    ...
    solver.set_subcatch_parameter(0, toolkit_enum.SubcatchProperty.WIDTH, 100)
    solver.set_subcatch_parameter(0, toolkit_enum.SubcatchProperty.AREA, 50)
    #solver.set_subcatch_parameter(0, toolkit_enum.SubcatchProperty.IMPERVIOUS_FRACTION, 100)
    solver.set_subcatch_parameter(0, toolkit_enum.SubcatchProperty.SLOPE, 0.1)
    solver.set_subcatch_parameter(0, toolkit_enum.SubcatchProperty.CURB_LENGTH, 10)
...
def test_get_subcatch_result(run_sim):
    rain = solver.get_subcatch_result(0, toolkit_enum.SubcatchResult.RAIN)
    evaporation = solver.get_subcatch_result(0, toolkit_enum.SubcatchResult.EVAPORATION)
    assert rain == 0.0
...
def test_get_subcatch_pollutant(run_sim):
    pollut = solver.get_subcatch_pollutant(0, toolkit_enum.SubcatchPollutant.BUILD_UP)
    tss, lead = tuple(pollut)
    assert tss == 0.0
    assert lead == 0.0
...
def test_lid_usage_parameter(lid_handle):
    unit_area = solver.get_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.UNIT_AREA)
    top_width = solver.get_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.TOP_WIDTH)
    #bottom_width = solver.get_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.BOTTOM_WIDTH)
    initial_saturation = solver.get_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.INITIAL_SATURATION)
    from_impervious = solver.get_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.FROM_IMPERVIOUS)
    from_pervious = solver.get_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.FROM_PERVIOUS)
    ...
    solver.set_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.UNIT_AREA, 100)
    solver.set_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.TOP_WIDTH, 20)
    #solver.set_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.BOTTOM_WIDTH, 20)
    solver.set_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.INITIAL_SATURATION, 10)
    solver.set_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.FROM_IMPERVIOUS, 50)
    solver.set_lid_usage_parameter(0, 0, toolkit_enum.LidUsageProperty.FROM_PERVIOUS, 10)
...
def test_lid_usage_option(lid_handle):
    index = solver.get_lid_usage_option(0, 0, toolkit_enum.LidUsageOption.INDEX) 
    number = solver.get_lid_usage_option(0, 0, toolkit_enum.LidUsageOption.NUMBER) 
    to_perv = solver.get_lid_usage_option(0, 0, toolkit_enum.LidUsageOption.TO_PERV) 
    drain_subcatch = solver.get_lid_usage_option(0, 0, toolkit_enum.LidUsageOption.DRAIN_SUBCATCH) 
    drain_node = solver.get_lid_usage_option(0, 0, toolkit_enum.LidUsageOption.DRAIN_NODE) 
    ...
    solver.set_lid_usage_option(0, 0, toolkit_enum.LidUsageOption.NUMBER, 200) 
...
def test_get_lid_usage_flux_rate(run_lid_sim):
    ...
    rate = solver.get_lid_usage_flux_rate(0, 0, toolkit_enum.LidLayer.SURFACE)
...
def test_get_lid_usage_result(run_lid_sim):
    ...
    result = solver.get_lid_usage_result(0, 0, toolkit_enum.LidResult.SURFACE_OUTFLOW)

    
def test_get_lid_group_result(run_lid_sim):
    index = solver.get_object_index(toolkit_enum.ObjectProperty.SUBCATCH, 'wBC')
    while True:
        time = solver.step()
        if time == 0:
            break

    result = solver.get_lid_group_result(index, toolkit_enum.LidResult.OLD_DRAIN_FLOW)
...
def test_lid_control_parameter(lid_handle):
    alpha = solver.get_lid_control_parameter(0, toolkit_enum.LidLayer.SURFACE, toolkit_enum.LidLayerProperty.ALPHA)
    surface_thickness = solver.get_lid_control_parameter(0, toolkit_enum.LidLayer.SURFACE, toolkit_enum.LidLayerProperty.THICKNESS)
    soil_thickness = solver.get_lid_control_parameter(0, toolkit_enum.LidLayer.SOIL, toolkit_enum.LidLayerProperty.THICKNESS)    
    storage_thickness = solver.get_lid_control_parameter(0, toolkit_enum.LidLayer.STORAGE, toolkit_enum.LidLayerProperty.THICKNESS)       
    ...
    solver.set_lid_control_parameter(0, toolkit_enum.LidLayer.SURFACE, toolkit_enum.LidLayerProperty.THICKNESS, 12)
    surface_thickness = solver.get_lid_control_parameter(0, toolkit_enum.LidLayer.SURFACE, toolkit_enum.LidLayerProperty.THICKNESS)
    assert surface_thickness == 12


def test_gage_precipitation(run_sim):
    total = solver.get_rain_precipitation(0, toolkit_enum.RainResult.TOTAL)
    snowfall = solver.get_rain_precipitation(0, toolkit_enum.RainResult.SNOWFALL)
    rainfall = solver.get_rain_precipitation(0, toolkit_enum.RainResult.RAINFALL)
    assert total == 0.0
    assert snowfall == 0.0
    assert rainfall == 0.0
